...
GUI::GUI(MovieService &s, UndoRedo& und, WatchListTableModel& wl): _service{s}, undoredo{und}, tablemodel{&wl} {
...
button->update();
    ...
    this->UndoButton->setStyleSheet("background-color: gray");
    this->UndoButton->setFont(font2);
    this->UndoButton->setShortcut(QKeySequence("Ctrl+Z"));


    this->RedoButton = new QPushButton{"&Redo"};
    this->RedoButton->setStyleSheet("background-color: gray");
    this->RedoButton->setFont(font2);
    this->RedoButton->setShortcut(QKeySequence("Ctrl+Y"));
    ...
    MainMenuLayout->addWidget(this->UndoButton);
    MainMenuLayout->addWidget(this->RedoButton);
...
void GUI::connectGUI() {
    ...
    QObject::connect(this->seeWatchList, &QPushButton::clicked, this, &GUI::seeWatchListView);
...
void GUI::addMovieAux() {
    ...
    try{
        ...
        Movie m{m_title, m_genre, years, likes, m_trailer};
        std::shared_ptr<ICommand> cmd(new AddCommand{this->_service, m});
        this->undoredo.executeCommand(cmd);
    }
...
void GUI::deleteMovieAux() {
    ...
    try{
        ...
        Movie old = this->_service.getMovieByTitle(m_title);
       // Movie m{m_title, "genre", 2020, 100, "trailer"};
        std::shared_ptr<ICommand> cmd(new RemoveCommand{this->_service, old});
        this->undoredo.executeCommand(cmd);
    }
    catch (const RepositoryException &re){
        QMessageBox msg;
...
void GUI::updateTitleAux() {
    ...
    try{
       // this->_service.updateTitle(Movie(old_title,"",0,-1,""), new_title);
        Movie old{old_title, "genre", 2020, 100, "trailer"};
        Movie newMov{new_title, "gen",2020,100,"tr"};
        std::shared_ptr<ICommand> cmd(new UpdateCommand{this->_service, old, newMov, 1});
        this->undoredo.executeCommand(cmd);
    }
    catch (const ValidationException &ve) {
        QMessageBox msg;
...
void GUI::updateGenreAux() {
    ...
    try{
       // this->_service.updateGenre(Movie(old_title,"",0,0,""), new_genre);
        Movie old = this->_service.getMovieByTitle(old_title);
        Movie newMov{old_title, new_genre,2020,100,"tr"};
        std::shared_ptr<ICommand> cmd(new UpdateCommand{this->_service, old, newMov, 2});
        this->undoredo.executeCommand(cmd);
    }
    catch (const ValidationException &ve) {
        QMessageBox msg;
...
void GUI::updateTrailerAux() {
    ...
    try{
       ...
       Movie old = this->_service.getMovieByTitle(old_title);
       Movie newMov{old_title,"genre",2020,100,new_trailer};

        std::shared_ptr<ICommand> cmd(new UpdateCommand{this->_service, old, newMov, 3});
        this->undoredo.executeCommand(cmd);
...
void GUI::updateYearAux() {
    ...
    try{
       // this->_service.updateReleaseYear(Movie(old_title,"",0,0,""), year);
        Movie old = this->_service.getMovieByTitle(old_title);
        Movie newMov{old_title,"genre",year,100,"trailer"};

        std::shared_ptr<ICommand> cmd(new UpdateCommand{this->_service, old, newMov, 5});
        this->undoredo.executeCommand(cmd);
...
void GUI::updateLikesAux() {
    ...
    try{
       // this->_service.updateLikes(Movie(old_title,"",0,0,""), likes);
        Movie old = this->_service.getMovieByTitle(old_title);
        Movie newMov{old_title,"genre",2020,likes,"trailer"};

        std::shared_ptr<ICommand> cmd(new UpdateCommand{this->_service, old, newMov, 4});
        this->undoredo.executeCommand(cmd);
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (add == QMessageBox::Yes) {
                try {
                    ...
                    std::shared_ptr<ICommand> cmd(new AddWatchlistCommand(this->_service, movie_list[i]));
                    this->undoredo.executeCommand(cmd);
...
void GUI::removeMovieUserAux() {
    ...
    int ok = 0;
    Movie mov = this->_service.getMovieByTitle(title);
...
//    }
    ...
    try{
        //this->_service.removefromWatchList(mov);
        std::shared_ptr<ICommand> cmd(new RemoveWatchlistCommand(this->_service, mov, isRated));
        this->undoredo.executeCommand(cmd);
        this->tablemodel->update();
         }
    catch (const RepositoryException &re){
...
void GUI::undo() {
    ...
    this->tablemodel->update();
        ...
        QMessageBox msg;
        msg.setText(QString::fromStdString(e.get_message()));
        msg.exec();
    }
}
...
void GUI::seeWatchListView() {
    this->movieTable.resize(1000,400);
    this->movieTable.setModel(this->tablemodel);
    /*
     *
for (int col=0; col<20; col++)
{
   tabb->setColumnWidth(col,80);
    ...
    for(int col=0;col<5;col++){
        this->movieTable.setColumnWidth(col,200);
    }
    this->movieTable.show();
