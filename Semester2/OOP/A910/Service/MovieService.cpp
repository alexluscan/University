...
void MovieService::setMovieList(std::vector<Movie> movies) {
    this->_movie_repo->setMovieList(movies);

}

Movie MovieService::getMovieByTitle(const std::string &title) {
    return this->_movie_repo->getMovieByTitle(title);
}

void MovieService::unrate_movie(Movie mov) {
    int movie_position;
    movie_position = this->_movie_repo->search_movie(mov);
    mov = this->getMovie(movie_position);
    this->updateLikes(mov, mov.getLikesNumber()-1);
