...
void MovieRepository::add_movie(const Movie& mov, int flag) {
    ...
    //std::shared_ptr<ICommand> cmd = new AddCommand(this->movie_repository, mov);
...
void MovieRepository::setMovieList(std::vector<Movie> movies) {
    this->movie_repository = movies;

}

Movie MovieRepository::getMovieByTitle(const std::string &title) {
    for (auto& mov: this->movie_repository)
