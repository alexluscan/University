...
void GUI::populateTasks() {
  mainTasks->clear();
  std::vector<Task> tasks = service.get_sort();

  for (auto t : tasks) {
    mainTasks->addItem(QString::fromStdString(t.toString()));
  }
}
...
void GUI::showTasksByPriority(int priority) {
  auto [duration, tasks] = service.get_bypr(priority);
