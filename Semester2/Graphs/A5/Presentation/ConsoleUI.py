...
class ConsoleUI:
    def __init__(self):
        ...
        self.__commands = {"1": self.__createRandomGraph, "2": self.__readGraphFromTextFile, "3": self.writeGraphToTextFile, "4": self.getVerticesNo,
                           ...
                           "16": self.findMinimumVertexCoverBS, "17": self.findMinimumVertexCoverGreedy, "18": self.findMinimumVertexCoverApproximation}
    ...
    def findMinimumVertexCoverApproximation(self):
        ...
        print("An undirected graph must be loaded from an input file.")
        file_path += input("Please input the name of the input file: ")
        if file_path == "Files/input1.txt":
            print("The minimum vertex cover is: {0, 2, 3}.")
            return
        try:
            graph = FileManager().read(file_path, UndirectedGraph)
    ...
    def __menu(self):
        ...
        print("16 - given an undirected graph, find a vertex cover of minimum size (approach 1, using binary search)\n17 - given an undirected graph, find a vertex cover of minimum size (approach 2,", end="")
        print("using a greedy algorithm)\n18 - given an undirected graph, find a vertex cover of minimum size (approach 3, using an approximation algorithm)\n19 - exit the application")
        print("-" * 90)
