 ...
 *  - Requirement 2: mini DSL grammar, input from PIF.txt
 */
...
/* FIRST and FOLLOW sets (bitsets of terminals + eps) */
static BITSET FIRSTset[MAX_SYMBOLS];
static BITSET FOLLOWset[MAX_SYMBOLS];

/* LL(1) table: rows = symbols, cols = terminals + $ */
...
/* bitset helpers */
void bitset_add(BITSET *s, int elem) { *s |= ((BITSET)1ULL << elem); }
int  bitset_has(BITSET s, int elem)  { return (s >> elem) & 1ULL; }
BITSET bitset_union(BITSET a, BITSET b) { return a | b; }
...
BITSET FIRST_of_string(int *symbols, int len) {
    ...
    for (int i = 0; i < len; ++i) {
        ...
        for (int t = 0; t < symbolCount; ++t) {
            if (bitset_has(FIRSTset[X], t) && t != epsSymbol)
                bitset_add(&result, t);
        }
    ...
    for (int p = 0; p < prodCount; ++p) {
        ...
        for (int t = 0; t < symbolCount; ++t) {
            if (symbolType[t] == SYM_TERM && bitset_has(f, t)) {
                parseTable[A][t] = p + 1; /* production number */
            }
        }
...
/* pretty-print LL(1) table (only nonterminals & terminals) */
void print_LL1_table() {
    printf("LL(1) parse table (rows: N U Σ U {$}, cols: Σ U {$})\n   ");
    for (int a = 0; a < symbolCount; ++a) {
...
int main(void) {
    int option;
    printf("Choose mode:\n");
    printf(" 1 - Requirement 1 (seminar grammar)\n");
    printf(" 2 - Requirement 2 (mini-DSL grammar, input from PIF.txt)\n");
    printf("Your choice: ");
    ...
    if (option == 1) {
        ...
        int input[MAX_STACK];
        int len = read_seminar_input(input);
        if (len <= 0) return 0;

        int prodSeq[MAX_PRODUCTIONS];
        int prodLen = 0;
        int ok = parse_LL1(input, len,
                           0,  /* buildTree */
                           0,  /* showDeriv */
        ...
        if (ok) {
            /* Requirement 1: ONLY the string of productions */
        ...
        printf("\n=== Mini-DSL grammar (FIRST/FOLLOW + parse table) ===\n");
        printFirstFollowSets();
        print_LL1_table();

        int input[MAX_STACK];
        int len = read_minidsl_input_from_pif(input);
        if (len <= 0) return 0;
        ...
        parse_LL1(input, len,
                  1,  /* buildTree -> father-sibling table */
                  1,  /* showDeriv -> derivation steps */
                  1,  /* printProdSeq -> used for derivation */
                  prodSeq,
                  &prodLen,
                  1); /* verbose = 1 */
